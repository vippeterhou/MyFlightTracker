<script lang="ts">
	...
	let { flights }: { flights: TrackedFlight[] } = $props();

	let localFlights = $state<TrackedFlight[] | null>(null);
	type PastView = 'grid' | 'timeline';
	const initialPastView =
		(browser && sessionStorage.getItem('pastView') as PastView) || 'grid';
	let pastView = $state<PastView>(initialPastView);
	let timelineMounted = $state(initialPastView === 'timeline');

	function setPastView(view: PastView) {
		pastView = view;
		if (view === 'timeline') timelineMounted = true;
	...
	let allFlights = $derived<TrackedFlight[]>(localFlights ?? flights);
	...
	{:else}
		...
		{#if pastFlights.length > 0}
			<div class="group">
				...
				<div class="view-content" class:hidden={pastView !== 'grid'}>
					<div class="grid">
				...
				{#if timelineMounted}
					<div class="view-content" class:hidden={pastView !== 'timeline'}>
