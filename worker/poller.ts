		...
		try {
			...
			if (prevStatus !== newStatus && NOTIFY_STATUSES.has(newStatus)) {
				const msg = buildNotification(flight.flightId, newStatus, {
					...
					scheduledDep: statusData.scheduledDep,
					actualDep: statusData.actualDep,
					estimatedDep: statusData.estimatedDep,
					departureTz: statusData.departureTz,
					wheelsOff: statusData.wheelsOff,
					wheelsOn: statusData.wheelsOn,
					scheduledArr: statusData.scheduledArr,
